...
def set_prompts_for_preferences(preferences):
    # Get current depth level
    current_depth = preferences["ai_email_bot"]["user preferences"]["depth"]
    ...
    while True:
        try:
            ...
            if new_depth not in [1, 2, 3]:
                ...
                continue
            preferences["ai_email_bot"]["user preferences"]["depth"] = new_depth
            break
        except ValueError:
    ...
    current_comm_styles = preferences["ai_email_bot"]["user preferences"]["communication_style"]
    ...
    while True:
        ...
        if all(style in valid_comm_styles for style in new_comm_styles):
            preferences["ai_email_bot"]["user preferences"]["communication_style"] = new_comm_styles
            break
        else:
    ...
    current_tone_styles = preferences["ai_email_bot"]["user preferences"]["tone_style"]
    ...
    while True:
        ...
        if all(style in valid_tone_styles for style in new_tone_styles):
            preferences["ai_email_bot"]["user preferences"]["tone_style"] = new_tone_styles
            break
        else:
    ...
    current_emoji_usage = preferences["ai_email_bot"]["user preferences"]["use_emojis"]
    ...
    while True:
        ...
        if new_emoji_usage.lower() in ['yes', 'no']:
            preferences["ai_email_bot"]["user preferences"]["use_emojis"] = new_emoji_usage.lower() == 'yes'
            break
        else:
            print("Invalid input. Please enter 'Yes' or 'No'.")

    # Get current language
    current_language = preferences["ai_email_bot"]["user preferences"]["language"]
    print(f"Current language is: {current_language}")

    new_language = input("Please enter a new language: ")
    preferences["ai_email_bot"]["user preferences"]["language"] = new_language
