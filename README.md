...
# refine the prompt with the ChatGPT conversational model. Use ChatGPT 4? Have setting the Preferences be optional. Design the preferences to be saved across user sessions. Determine if AgentGPT is in usage while you just write emails. Must be able to run through write function and modify function multiple times once preferences are preserved. Also either have ChatGPT write the prompt for you or do not have ChatGPT write the prompt. Make the emails send emojis. 
